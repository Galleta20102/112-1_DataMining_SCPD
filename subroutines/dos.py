...
from DOS import moment_comp, moment_filter, rescale_matrix, plot_cheb
import sys
sys.path.append('../')
...
def get_dos(L, Nmoment=50, nZ=100, outname="none", npts=20, maxsize=-1, compute_range=False, adj=False):
    if (not adj):
        if (compute_range):
            ...
            # the range is specified by normalized Laplacian
            L, _ = rescale_matrix.rescale_matrix(L, range=[0, 2])
    ...
    if (outname == "none"):
        ...
        if (maxsize != -1):
            xm, yy = plot_cheb.plot_chebhist((c,), pflag=True, npts=(
                npts+1), outname=outname, maxsize=maxsize)
        else:
            xm, yy = plot_cheb.plot_chebhist(
                (c,), pflag=True, npts=(npts+1), outname=outname)
...
def mag_history_dos(Nmoment=10, nZ=20, npts=50):
    ...
    print("there are " + str(int(total_edges/len(G_times))) +
          " average edges per snapshot")
...
def airport_history_dos(Nmoment=10, nZ=20, npts=50):
    ...
    directed = False
    G_times = geology_loader.load_temporarl_edgelist2(fname)
    vecs = []
    density = []
    total_edges = 0
    total_time = 0
    ...
    for i in range(len(G_times)):
        ...
        start = timeit.default_timer()
        vecs.append(get_dos(L, Nmoment=Nmoment, nZ=nZ, npts=npts))
        normal_util.save_object(
            vecs, experimentsName + 'N' + str(Nmoment)+".pkl")
        end = timeit.default_timer()
        total_time = total_time + end - start
    print('DOS time: '+str(total_time)+'\n')
    print("total nodes are " + str(total_nodes))
...
def Laplace_dos(fname, Nmoment=50, nZ=50, npts=20, svd=False, compute_range=False, save=True):
    ...
    for i in range(len(G_times)):
        ...
        start = timeit.default_timer()
        if (i in cps):
            vecs.append(get_dos(L.copy(), Nmoment=Nmoment, nZ=nZ,
                        npts=npts, outname="none", compute_range=compute_range))
        else:
            vecs.append(get_dos(L.copy(), Nmoment=Nmoment, nZ=nZ,
                        npts=npts, outname="none", compute_range=compute_range))
        end = timeit.default_timer()
        ...
        if (svd):
            ...
            if (i in cps):
                vals = get_svd(L)
                counts, pos, _ = plt.hist(
                    vals, bins=20, range=(0, 2), color="#3690c0")
                for k in range(len(counts)):
                    plt.text(pos[k], counts[k] + 1, str(math.ceil(counts[k])),
            ...
            if (i in cps):
                counts, pos, _ = plt.hist(
                    vals, bins=20, range=(-1, 1), color="#3690c0")
                for k in range(len(counts)):
                    plt.text(pos[k], counts[k] + 1, str(math.ceil(counts[k])),
                             color='black', fontweight='bold')
    ...
    if (save):
        normal_util.save_object(vecs, "dosNm" + str(Nmoment) +
                                "Nz" + str(nZ) + "Npts" + str(npts) + fname + ".pkl")
        if (svd):
...
def coin_dos(Nmoment=10, nZ=20, npts=50):
    # "../datasets/coin/coin_edgelist.csv"
    fname = "subroutines\coin_edgelistv3.csv"
    outname = "coindosv3"
...
if __name__ == '__main__':
    ...
    nZ = 100
    npts = 50
    airport_history_dos(Nmoment=Nmoment, nZ=nZ, npts=npts)
    ...
    for seed in seeds:
        ...
        # compute dos
        vecs, vname, time = Laplace_dos(
            dname, Nmoment=Nmoment, nZ=nZ, npts=npts, svd=svd, compute_range=compute_range)

        # find anomalies
        _, accu = Anomaly_Detection.multi_SBM(
            vname, real_events=real_events, verbose=False)
        accus.append(accu)
        times.append(time)
