...
def multi_SBM(fname, verbose=True, real_events=[16,31,61,76,91,106,136], timestamps=151, n_init=20):
    ...
    accu = metrics.accuracy_score(anomalies, real_events)
...
def airport_geology(fname, r1, r2, outname="airport_Geology"):
    ...
    percent_ranked = 0.05
    real_events = []
    ds = list(range(1990+r1,1990+r1+r2,12))
    dates=[]
    dates_idx=[]
    for d in ds:
        m=(d-1990)%12+1
        y=ceil(((d-1990)/12+1990))
        dates.append(f"{y}-{m}")          
    #dates = [(str(d)+str(d)) for d in ds]
    dates_idx = list(range(0,r2-r1,12))
    ...
    for d in events:
        m=d%12+1
        y=floor(((d)/12+1990))
        events2.append(f"{y}-{m}")  
    print(f'event2={events2}')
    scores = z_scores
    score_labels = ["window size "+ str(window) for window in windows]

    event_labels = [str(e) for e in events2]
    #event_labels = [str(e + 1990 + r1) for e in events]
    print (f'event labels are {event_labels}')

    diag_vecs = normal_util.load_object(fname)
    diag_vecs = diag_vecs[r1:r2]
    plot_dblp_spectro2(outname, scores, score_labels, events, event_labels, diag_vecs, dates_idx, dates, initial_window=max(windows))
    return z_overall
...
def main():
    ...
    '''
    fname = "skynetgdos.pkl"
    #fname = "china.pkl"
    percent_ranked = 0.07
    SkyNet(fname, outname="covid_flight", percent_ranked=percent_ranked)

    '''
    run this block of code for USA airport dataset
    '''
    fname2 = "airport_USAflightN20.pkl"
    outname2 = "airport_USAflight"
    airport_geology(fname2, 0, 240, outname=outname2)
